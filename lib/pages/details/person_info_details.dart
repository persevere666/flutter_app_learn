...
class PersonInfoDetailsPage extends StatefulWidget {
 ...
 class _PersonInfoDetailsPageState extends State<PersonInfoDetailsPage> {
  ...
  /// 签名焦点
  final FocusNode _mottoFocusNode = FocusNode();
  /// 兴趣爱好焦点
  ...
  void initState() {
    ...
    _initData(
        ...
        user?.userAddress??'',
        user?.userBorth??'',
        (user?.facultyName??'') + '-' + (user?.specialtyName??'')
    );
  }
      ...
      onTap: !_enableEdit ? null : () async{
        ...
        Picker(
          adapter: PickerDataAdapter<String>(
              pickerData: JsonDecoder().convert(_facultyAndSpecialty)
          ),
          changeToFirst: true,
          hideHeader: false,
    ...
    return ListTile(
      ...
      onTap: !_enableEdit ? null : () async {
        ...
        final String? result = await CityPickers.showCityPicker(
          ...
          if(data.cityName == null || data.provinceName==null) return null;
          print('provinceName = ${data.provinceName}, cityName=${data.cityName}');
     ...
     // 适配设置
         ...
         /// 
          return Consumer<LoginProvider>(builder: (context,loginProvider,child){
            ...
            return (!loginProvider.isLogin) || user == null
                ? const Center(child: Text('校园信息服务平台出错了！'))
                : Scaffold(
                    appBar: AppBar(
                          ...
                          child: ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                final Map<String,dynamic> data = {
                                  "userId": user.userId,
                                  "nickname": _nickname != null && _nickname != '' && _nickname != user.userNickname ? _nickname : null,
                                  "motto": _motto != null && _motto != '' && _motto != user.userMotto ? _motto : null,
                                  "hobby": _hobby != null && _hobby != '' && _hobby != user.userHobby ? _hobby : null,
                                  "birthday":  _userBirthday != null && _userBirthday != ''&& _userBirthday != user.userBorth ? _userBirthday : null,
                                  "address": _userAddress != null && _userAddress != '' && _userAddress != user.userAddress ? _userAddress : null,
                                  "yxzy": _yxzy != null && _yxzy != '' ? _yxzy : ('${user.facultyName}-${user.specialtyName}'),
                                };
                                print('修改的待提交的数据: ${data.toString()}');
                              /// 向服务器传送数据完成修改
                              final ResponseData? resData = await BjuHttp.put(
                                  API.updateUserInfo, params: data
                              ).then((onValue){
                                  try {
                            ...
                            children: [
                              ...
                              ListTile(
                                ...
                                trailing: TextButton.icon(
                                  ...
                                  label: const Text('换头像',style: TextStyle(
                                    fontSize: 16,
