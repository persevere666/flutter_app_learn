...
class DBUtil {
  ...
  /// 数据库名称
  static const String _DB_NAME = 'BJU.db';

  /// 数据库中表的名称
  static const String _TABLE_NAME = 'user_message';

  /// 建表的SQL语句 (与UserMessage字段基本一致，多了owner字段，该字段标识UserMessage所属的用户)
  static const String _CREATE_TABLE_SQL = '''
  ...
  static Future<Database> openDB() async {
    ...
    // 打开并创建数据库
    final Database db = await openDatabase(dbPath, version: _DB_VERSION,
  ...
  static Future<bool> insertMessage(Map<String, dynamic> values) async {
    ...
    // 插入数据到SQLLite表中
    int count = await db.insert(_TABLE_NAME, values,
  ...
  /// *__返回执行的结果（true?false）__
  ///
  static Future<bool> readMessagesWithTypeAndOwner(
    ...
    // 标记表中的数据为已读状态
    int count = await db.update(_TABLE_NAME, {'read': 'true'},
        where: 'owner = ? and messageType = ?', whereArgs: [owner, type]);
    // 关闭数据库
    close();
  ...
  ///
  static List<UserMessage> transformList2UserMessage(
      List<Map<String, dynamic>> mapList) {
    // map封装为具体类型
    final List<UserMessage> list =
        mapList.map<UserMessage>((m) => UserMessage.fromJson(m)).toList();
